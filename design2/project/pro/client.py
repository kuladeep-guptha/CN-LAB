...
class ServerConnection(QThread):
    ...
    def handle_main(self, conn):
        global all_clients, active_clients       #doubt
        while self.connected:
            msg_bytes = conn.recv_bytes()
            if not msg_bytes:
                self.connected = False   #doubt
                break
            ...
            if type(msg) != Message:
                ...
                all_clients[client_name] = Client(client_name, "addr") #doubt
    ...
    def multicast_msg(self, conn, msg):
        ...
        while self.connected:
            ...
            current_msg.from_name = client.name       #doubt
