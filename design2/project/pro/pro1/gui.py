...
class Audio:
    def __init__(self):
        self.audio_stream = pa.open(rate=55000, channels=1, format=pyaudio.paInt16, input=True, frames_per_buffer=4096)

    def get_stream(self):
        if not ENABLE_AUDIO:
            return None
        self.audio = self.audio_stream.read(4096, exception_on_overflow=False)
        return self.audio
...
class PlayAudio(QThread):
    def __init__(self, client):
        ...
        self.connected = True
        self.client = client
        self.audio_stream = pa.open(rate=55000, channels=1, format=pyaudio.paInt16, output=True, frames_per_buffer=4096)
...
class VideoListWidget(QListWidget):
    ...
    def add_toggle_buttons(self):
        ...
        self.video_widget.layout.addLayout(h_layout)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        if not login_window1.exec():
            print("Login cancelled")
            self.close()
            exit(0)
        else:
            self.client.name = login_window1.name
        ...
        self.chat=ChatWidget()
        self.add_toggle_buttons1()   # add video & audio on/off, end-call buttons
        self.tab.addTab(self.video_list,"video")
    ...
    def toggle_camera(self):
        ...
        print(f"Camera Enabled: {self.client.camera_enabled}")
    ...
    def toggle_microphone(self):
        ...
        print(f"Microphone Enabled: {self.client.microphone_enabled}")
    

    def add_toggle_buttons1(self):
        self.end_call = QPushButton("Leave Call")
        # change its color to red and round edges
        # self.end_call.setStyleSheet("background-color: #F15A59;")
        self.end_call.clicked.connect(self.close)
    ...
    def add_client(self, client):
        self.video_list.add_video(client)                         #video frame on screen
        self.audio_threads[client.name] = PlayAudio(client)
        self.audio_threads[client.name].start()                  #audio thread
    ...
    def remove_client(self, client_name):
        ...
        self.chat.chat_box.append(f"{client_name} left the conference")
        ...
        if self.client.name != msg.from_name:
            ...
            if msg.request != 'join':
                ...
                for name in msg.to_names:
                    chat += name + ', '
                chat = chat[:-2] + ' ]'
        self.chat.chat_box.append(chat)